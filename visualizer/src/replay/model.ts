...
export function buildReplay(payloadValue: unknown, exportedValue: unknown): ReplayModel {
  ...
  const frames: ReplayFrame[] = [frame(board, current, 0)];
  for (const turn of payload.turns) {
    ...
    frames.push(frame(board, current, frames.length, turn.action));
  }
  const final = frame(board, null, frames.length - 1, frames.at(-1)?.lastAction);
  ...
  return Object.freeze({ frames: Object.freeze(frames.map((value) => Object.freeze(value))), turns: Object.freeze([...payload.turns]), final: Object.freeze(final) });
}
