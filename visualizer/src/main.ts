...
function render(model: ReplayModel, base?: string, selectedMatch?: string) {
  ...
  const controls = base ? `<section class="discovery" aria-label="Replay discovery controls"><p>Viewing completed match ${escape(selectedMatch ?? "")}</p><button id="change-match" type="button">Choose another match</button></section>` : "";
