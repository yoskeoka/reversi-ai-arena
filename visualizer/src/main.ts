...
function renderDiscovery(base?: string, selectedMatch?: string, matches: PublicMatch[] = [], error?: string) {
  ...
  const matchOptions = matches.map((match) => `<option value="${escapeAttribute(match.match_id)}"${match.match_id === selectedMatch ? " selected" : ""}>${escape(match.match_id)}</option>`).join("");
