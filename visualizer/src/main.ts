import "./style.css";
import { baseProfiles, listCompletedMatches, loadReplay, matchOptionLabel, normalizeBaseUrl, shortRevision, type PublicMatch } from "./api/public-api";
...
async function start() {
  ...
  try { const matches = await listCompletedMatches(base); if (current !== generation) return; renderDiscovery(base, matchId, matches); if (matchId) try { const replay = await loadReplay(base, matchId); if (current === generation) render(replay.model, base, matchId, replay.match); } catch (error) { if (current === generation) renderDiscovery(base, matchId, matches, errorMessage(error)); } }
  catch (error) { if (current === generation) renderDiscovery(base, matchId, [], errorMessage(error)); }
}
...
function render(model: ReplayModel, base?: string, selectedMatch?: string, match?: PublicMatch) {
  ...
  document.querySelector<HTMLButtonElement>("#change-match")?.addEventListener("click", () => { if (playing) clearInterval(playing); updateQuery(base!); });
